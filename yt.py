#Jangan Di Decode Ngab:)
#Buat Logo Aja Udah Susah Apalagi Tools Nya:)
#Copyright By AmmarBN
#Dibuat Pada ©2022 Sunday 20 Feb
...
if __name__=="__main__":
    ...
    if choice == "mp3":
       ...
       print(f"{W}[{G}DONE{W}]\033[00m file tersimpan di,\n-> {G}/sdcard/Download/undifined 404")
        ...
        sys.exit(f"{W}[{R}INFO{W}]\033[00mInvalid Input Choice{R}!{W}")
    cblg=input(f"{W}[{G}?{W}]\033[00mCoba Lagi? {W}({G}y{W}/{G}n{W}): {G}")
    if cblg == "y" or cblg == "Y":
        sleep(2)
        os.system("python yt.py")
    else:
        sys.exit(f"{W}[{R}EXIT{W}]THX FOR USING SCRIPT{R}!!{W}")
